...
class ShowHistoryDistanceController extends SuperController {
  ...
  void onInit() async {
    ...
    ever(serviceController.list, (list) async {
      ...
      if (list != null && list.isNotEmpty) {
        initialCameraPosition.value = LatLng(list[0].latitude.toDouble(), list[0].longitude.toDouble());
        data.value = list;
  ...
  Future<void> createMarkersAndPolylines(List<DataModel> list) async {
    ...
    for (int i = 0; i < list.length; i++) {
      LatLng position = LatLng(list[i].latitude.toDouble(), list[i].longitude.toDouble());
      points.add(position);
      markers.add(
        Marker(
